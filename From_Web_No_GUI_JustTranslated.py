...
def buildPresentation():
    ...
    def just_translation(text, dest_language):
        url = "https://just-translated.p.rapidapi.com/"
        querystring = {"lang": dest_language,"text": text}

        headers = {
	        "X-RapidAPI-Key": "728568a0f9msh9ca208bfe9f574cp15a334jsn93b6687280e7",
        ...
        if response.status_code == 200:
            translated_text = str(response.json()["text"])
    ...
    for i in range(0, presentationLength):
        ...
        if do_translation:
            printDescription = (just_translation(printDescription, dest_language))
            printFact = (just_translation(printFact, dest_language))
